"""
Config loader for the Kanberoo TUI.

Reads ``$KANBEROO_CONFIG_DIR/config.toml`` (default
``~/.kanberoo/config.toml``) and applies the same environment-variable
...
``kanberoo_cli.config`` module rather than a re-export. Coupling two
...
def load_config(path: Path | None = None) -> TuiConfig:
    ...
    ``$KANBEROO_API_URL`` and ``$KANBEROO_TOKEN`` each override the
