"""
Baseline SVG snapshot coverage for the Kanberoo TUI.
...
packages/kanberoo-tui/tests/test_snapshots.py``.

Two host shapes
---------------

* **Screens** are rendered through a full :class:`KanberooTuiApp` with
...
def app_factory(
    ...
    fake_editor: Any,
) -> Callable[..., KanberooTuiApp]:
    """
    Build a :class:`KanberooTuiApp` pre-wired to the fixture-scoped
    fakes. Used by every screen-level snapshot test.
    """

    def _factory(**overrides: Any) -> KanberooTuiApp:
        return KanberooTuiApp(
            config=tui_config,
