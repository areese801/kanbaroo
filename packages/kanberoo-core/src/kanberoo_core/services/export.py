...
* ``kanberoo.db`` -a fresh SQLite file with the same schema and the
...
from kanberoo_core.migrations import build_alembic_config
...
from kanberoo_core.services.workspaces import get_workspace
...
def _arrow_schema_for_table(table: Table) -> pa.Schema:
    ...
    self-describing and to round-trip the TEXT/INTEGER storage Kanberoo
    uses end to end.
    """
...
def _build_sqlite_copy(
    ...
    The database is created with the full Kanberoo schema (via
...
def export_workspace(
    ...
    *,
    ...
    Raises :class:`~kanberoo_core.services.exceptions.NotFoundError`
