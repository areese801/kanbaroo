"""
Shared fixtures for the Kanbaroo TUI test suite.
...
* :class:`FakeWsStream` and its factory helper let tests script a
  ...
  the real :class:`~kanbaroo_tui.ws.EventSubscriber`.
* :class:`FakeEditor` replaces the ``$EDITOR`` subprocess inside
  :mod:`kanbaroo_tui.editor`. The test writes whatever it wants into
...
from kanbaroo_tui.client import API_PREFIX, AsyncApiClient
...
class FakeEditor:
    ...
    :class:`KanbarooTuiApp` (or a screen constructed by hand).
    """
