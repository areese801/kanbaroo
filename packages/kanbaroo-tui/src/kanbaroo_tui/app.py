"""
Kanbaroo TUI application.

The :class:`KanbarooTuiApp` owns the long-lived HTTP client and the
...
from kanbaroo_tui.config import (
    ConfigError,
    ConfigNotFoundError,
    TuiConfig,
    load_config,
)
from kanbaroo_tui.editor import EditorRunner
from kanbaroo_tui.messages import (
    EpicSelected,
    OpenAuditFeed,
    OpenEpicList,
    OpenSearch,
...
from kanbaroo_tui.screens.audit_feed import AuditFeedScreen
from kanbaroo_tui.screens.board import BoardScreen
from kanbaroo_tui.screens.epic_detail import EpicDetailScreen
from kanbaroo_tui.screens.epic_list import EpicListScreen
from kanbaroo_tui.screens.search import SearchScreen
...
from kanbaroo_tui.ws import EventSubscriber, build_events_url
...
class KanbarooTuiApp(App[None]):
    """
    Top-level Textual app for Kanbaroo.
...
def main() -> None:
    """
    Synchronous entry point for the ``kanbaroo-tui`` console script.
    ...
    try:
        ...
        print(
            f"Kanbaroo config not found at {exc.path}.\nRun `kb init` to create one.",
            file=sys.stderr,
        )
        sys.exit(1)
    except ConfigError as exc:
        print(f"Kanbaroo config error: {exc}", file=sys.stderr)
        sys.exit(1)
    app = KanbarooTuiApp(config=config)
    app.run()
