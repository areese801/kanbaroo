...
Creates ``$KANBEROO_CONFIG_DIR`` (defaulting to ``~/.kanberoo``), runs
...
from kanberoo_core.db import engine_for_url
from kanberoo_core.migrations import upgrade_to_head
...
def _default_database_url(config_dir: Path) -> str:
    """
    Resolve the database URL from ``KANBEROO_DATABASE_URL`` or fall
    back to a SQLite file inside ``config_dir``.
    """
    override = os.environ.get("KANBEROO_DATABASE_URL")
    if override:
        return override
    return f"sqlite:///{config_dir / 'kanberoo.db'}"
...
def init(
    ...
    """
    Initialise Kanberoo's local config, database, and first API token.
    """
