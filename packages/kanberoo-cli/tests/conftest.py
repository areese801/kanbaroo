"""
Shared fixtures for the Kanberoo CLI test suite.
...
:func:`kanberoo_cli.context.set_client_factory` keeps command handlers
...
from kanberoo_cli.client import API_PREFIX, ApiClient
from kanberoo_cli.context import set_client_factory
...
def config_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """
    Create a temporary Kanberoo config directory with a minimal
    ``config.toml`` pointing at a fake server, and redirect
    ``$KANBEROO_CONFIG_DIR`` there.
    """
    ...
    (tmp_path / "config.toml").write_text(
        ...
        'token = "kbr_test"\n'
        f'database_url = "sqlite:///{tmp_path / "kanberoo.db"}"\n',
        encoding="utf-8",
    )
    return tmp_path
