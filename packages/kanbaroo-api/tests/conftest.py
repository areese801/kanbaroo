"""
Fixtures for the ``kanbaroo-api`` integration tests.

Each test gets a fresh FastAPI app backed by an in-memory SQLite
database. The database is stood up via
:func:`kanbaroo_core.migrations.upgrade_to_head` so tests exercise the
...
from kanbaroo_core import ActorType, models  # noqa: F401 (registers tables)
