...
These tests run only when ``kanbaroo-web`` is installed (the standard dev
...
kanbaroo_web = pytest.importorskip("kanbaroo_web")
...
def test_ui_root_returns_placeholder(client: TestClient) -> None:
    ...
    assert "Kanbaroo" in response.text
...
def test_web_assets_path_points_at_bundled_dist() -> None:
    ...
    """
    assets = kanbaroo_web.web_assets_path()
