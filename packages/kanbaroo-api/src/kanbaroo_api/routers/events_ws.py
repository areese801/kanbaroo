...
every event published on :data:`kanbaroo_core.events.default_bus`.
...
:class:`~kanbaroo_core.events.Event` envelope: it carries
...
from kanbaroo_core.events import default_bus
...
async def _forward_events(websocket: WebSocket) -> None:
    ...
    Each :class:`~kanbaroo_core.events.Event` is dumped to its dict
...
async def _keepalive(websocket: WebSocket, interval: float) -> None:
    ...
    NOT share a shape with :class:`~kanbaroo_core.events.Event` so
