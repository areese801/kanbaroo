...
CLI config (or ``$KANBAROO_DATABASE_URL``) and copies it to a
timestamped path under ``--output`` (default ``~/.kanbaroo/backups``).
...
from kanbaroo_cli.config import default_config_dir
from kanbaroo_cli.context import require_config
...
def backup_command(
    output: Path | None = typer.Option(
        None,
        "--output",
        help="Directory to write the backup into. Default: ~/.kanbaroo/backups.",
    ),
) -> None:
    """
    ...
    dest = dest_dir / f"kanbaroo-{timestamp}.db"
