...
def _ws_body(key: str = "KAN", name: str = "Kanbaroo") -> dict[str, object]:
    """
...
def test_list_workspaces_renders_table(
    ...
    mock_api.json(
        "GET",
        "/workspaces",
        body={"items": [_ws_body("KAN", "Kanbaroo")], "next_cursor": None},
    )
    result = runner.invoke(app, ["workspace", "list"])
    assert result.exit_code == 0, result.stderr
    assert "KAN" in result.stdout
    assert "Kanbaroo" in result.stdout
...
def test_create_workspace_posts_payload(
    ...
    mock_api.json(
        "POST",
        "/workspaces",
        body=_ws_body("KAN", "Kanbaroo"),
        status_code=201,
        headers={"etag": "1"},
    )
    result = runner.invoke(
        app,
        ["workspace", "create", "--key", "KAN", "--name", "Kanbaroo"],
    )
    assert result.exit_code == 0, result.stderr
    assert mock_api.requests[-1].method == "POST"
    assert mock_api.requests[-1].path == "/workspaces"
    assert mock_api.requests[-1].body == {"key": "KAN", "name": "Kanbaroo"}
...
def test_workspace_current_reports_unset(
    ...
    """
    del config_dir
    monkeypatch.delenv("KANBAROO_WORKSPACE", raising=False)
...
def test_workspace_current_reports_env(
    ...
    """
    ``$KANBAROO_WORKSPACE`` wins over any config value.
    """
    del config_dir
    monkeypatch.setenv("KANBAROO_WORKSPACE", "FROM_ENV")
    result = runner.invoke(app, ["workspace", "current"])
    assert result.exit_code == 0, result.stderr
...
def test_story_list_uses_flag_over_env_and_config(
    ...
    """
    The ``--workspace`` flag wins over both ``$KANBAROO_WORKSPACE`` and
    ``default_workspace`` in config.
    """
    (config_dir / "config.toml").write_text(
        'api_url = "http://test.invalid"\n'
        'token = "kbr_test"\n'
        f'database_url = "sqlite:///{config_dir / "kanbaroo.db"}"\n'
...
def test_story_list_uses_env_over_config(
    ...
    """
    ``$KANBAROO_WORKSPACE`` wins over ``default_workspace`` in config
    ...
    monkeypatch.setenv("KANBAROO_WORKSPACE", "ENVKEY")
    mock_api.json(
        "GET",
...
def test_story_list_without_workspace_errors_cleanly(
    ...
    assert "KANBAROO_WORKSPACE" in combined
    del config_dir
