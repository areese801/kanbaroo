...
def test_backup_copies_sqlite_file(
    ...
    config_dir.mkdir()
    monkeypatch.setenv("KANBAROO_CONFIG_DIR", str(config_dir))
    monkeypatch.delenv("KANBAROO_DATABASE_URL", raising=False)
    db_path = tmp_path / "db" / "kanbaroo.db"
    ...
    produced = list(out_dir.glob("kanbaroo-*.db"))
...
def test_backup_warns_on_non_sqlite_url(
    ...
    monkeypatch.setenv("KANBAROO_CONFIG_DIR", str(config_dir))
    monkeypatch.delenv("KANBAROO_DATABASE_URL", raising=False)
    _write_config(config_dir, "postgresql://localhost/kanbaroo")
