"""
Tests for :func:`kanbaroo_core.services.export.export_workspace`.
...
from kanbaroo_core.services.export import (
    EXPORT_TABLE_NAMES,
    export_filename_for,
    export_workspace,
)
...
def test_export_archive_contains_expected_members(session: Session) -> None:
    ...
    expected_members = {"schema_version.json", "kanbaroo.db"} | {
...
def test_export_sqlite_copy_has_only_workspace_rows(session: Session) -> None:
    """
    The embedded ``kanbaroo.db`` carries the full schema (minus
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        db_path.write_bytes(members["kanbaroo.db"])
