"""
Tests for :func:`kanbaroo_core.services.audit.list_audit` and
:func:`kanbaroo_core.services.audit.list_audit_for_entity`.
...
from kanbaroo_core.services.audit import (
    emit_audit,
    list_audit,
...
def test_list_audit_since_skips_older_rows(session: Session) -> None:
    ...
    first_row = session.get(
        __import__("kanbaroo_core.models.audit", fromlist=["AuditEvent"]).AuditEvent,
        ids[0],
    )
