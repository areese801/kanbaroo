...
:mod:`kanbaroo_core.services.workspaces` and assert the load-bearing
...
from kanbaroo_core.models.workspace import Workspace
...
def test_update_workspace_emits_audit_with_before_and_after(
    ...
    updated = ws_service.update_workspace(
        ...
        payload=WorkspaceUpdate(name="Kanbaroo Renamed"),
    )
    session.commit()
    ...
    assert diff["before"]["name"] == "Kanbaroo"
    assert diff["after"]["name"] == "Kanbaroo Renamed"
