"""
Alembic environment for the Kanbaroo core schema.

Reads ``sqlalchemy.url`` from ``alembic.ini`` by default; the env var
``KANBAROO_DATABASE_URL`` overrides it when set. Importing
:mod:`kanbaroo_core.models` registers every table on
...
from kanbaroo_core import models  # noqa: F401  (registers tables on Base.metadata)
...
_url_override = os.environ.get("KANBAROO_DATABASE_URL")
if _url_override:
