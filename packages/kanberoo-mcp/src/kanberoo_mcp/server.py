"""
Kanberoo MCP server entry point.

The server is a thin translator between the MCP tool protocol and the
Kanberoo REST API. It runs as a subprocess of the outer client
...
* Resolve configuration via :func:`kanberoo_mcp.config.resolve_config`.
* Detect whether the resolved token is actor_type=claude; log a
  warning if not, but never gate startup on it.
* Register every tool defined in :func:`kanberoo_mcp.tools.build_registry`
...
from kanberoo_mcp import __version__
from kanberoo_mcp.client import (
    McpApiClient,
    McpApiError,
    McpApiRequestError,
)
from kanberoo_mcp.config import ConfigError, McpConfig, resolve_config
from kanberoo_mcp.tools import ToolRegistry, build_registry

_LOG = logging.getLogger("kanberoo_mcp")
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="kanberoo-mcp",
        description=(
            "Kanberoo MCP server: serves the Kanberoo REST API as MCP tools over stdio."
        ),
    )
    parser.add_argument(
        "--api-url",
        ...
        default=None,
        help=(
            "Base URL of the Kanberoo API (e.g. http://localhost:8080). "
            "Falls back to $KANBEROO_API_URL then to config.toml."
        ),
    )
    parser.add_argument(
        "--token-env",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"kanberoo-mcp {__version__}",
    )
...
def _warn_if_non_claude_token(client: McpApiClient, config: McpConfig) -> None:
    ...
    if actor_type == "claude":
        _LOG.info(
            "kanberoo-mcp ready: api_url=%s token_source=%s actor_type=claude",
            config.api_url,
            config.token_source,
        )
        return
    _LOG.warning(
        "kanberoo-mcp token has actor_type=%s (source=%s). Mutations "
...
def build_server(
    ...
    server: Server[object, object] = Server(
        ...
        version=__version__,
        instructions=(
            "Kanberoo issue tracker. Use list_workspaces and "
...
def _configure_logging() -> None:
    ...
    DEBUG via $KANBEROO_MCP_LOG_LEVEL.
    """
    import os

    level_name = os.environ.get("KANBEROO_MCP_LOG_LEVEL", "INFO").upper()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    Returns the process exit code; the ``kanberoo-mcp`` wrapper in
    ...
    try:
        ...
        print(f"kanberoo-mcp: {exc}", file=sys.stderr)
        return 1
