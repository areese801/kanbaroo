...
silently swap, say, $KANBEROO_TOKEN ahead of --token.
"""
...
from kanberoo_mcp.config import ConfigError, resolve_config
...
def test_resolve_config_prefers_cli_flags(tmp_path: Path) -> None:
    ...
    cfg = resolve_config(
        ...
        cli_token="from-flag",
        cli_token_env=None,
        env={"KANBEROO_API_URL": "http://env.invalid", "KANBEROO_TOKEN": "from-env"},
...
def test_resolve_config_uses_token_env_indirection(tmp_path: Path) -> None:
    ...
    cfg = resolve_config(
        cli_api_url="http://flag.invalid",
        cli_token=None,
        cli_token_env="KANBEROO_MCP_TOKEN",
        env={"KANBEROO_MCP_TOKEN": "claude-token"},
        config_path=tmp_path / "missing.toml",
    )
    assert cfg.token == "claude-token"
    assert cfg.token_source == "$KANBEROO_MCP_TOKEN"
...
def test_resolve_config_falls_back_to_shared_cli_token(tmp_path: Path) -> None:
    ...
    ``$KANBEROO_TOKEN`` (the CLI's shared token env var).
    """
    cfg = resolve_config(
        cli_api_url=None,
        cli_token=None,
        cli_token_env=None,
        env={
            "KANBEROO_API_URL": "http://env.invalid",
            "KANBEROO_TOKEN": "human-cli-token",
        },
        config_path=tmp_path / "missing.toml",
    )
    assert cfg.api_url == "http://env.invalid"
    assert cfg.token == "human-cli-token"
    assert cfg.token_source == "$KANBEROO_TOKEN"
...
def test_resolve_config_missing_token_is_fatal(tmp_path: Path) -> None:
    ...
    assert "KANBEROO_MCP_TOKEN" in message
    assert "KANBEROO_TOKEN" in message
